<section class="main-map">
    <div class="container">
        <div class="main-map__content">
            <div class="main-map__title">
                <h1>
                    <span class="main-map__title-primary">Перевозки грузов</span>
                    <a class="main-map__title-secondary main-map__title-secondary--accent"
                        href="{{ route('pages.show', ['page' => 'zhd-perevozki']) }}">
                        железнодорожным
                    </a>
                    <span class="main-map__title-secondary">
                        , автомобильным, морским транспортом
                    </span>
                </h1>
                <span class="main-map__title-secondary main-map__title-secondary--add"> из любой
                    точки России, СНГ, Китая
                </span>
            </div>
        </div>
    </div>
</section>
